...
开启后，仅对 R18/R18G 作品的图片进行像素混淆加密处理，全年龄作品不受影响。用于降低发送时被系统风控拦截或封禁的风险（如果被举报可能会导致封号）。群聊默认开启，私聊默认关闭，可分别通过 `pixiv_encrypt_image_group` 和 `pixiv_encrypt_image_private` 配置。
