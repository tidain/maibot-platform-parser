...
本版本在原移植版基础上扩展了全部16个平台的解析支持，并添加了管理员命令、B站扫码登录等功能。
...
```toml
[parser]
enabled_platforms = ["bilibili", "douyin", "xhs", "xiaoheihe", "acfun", "kuaishou", "ncm", "nga", "shipinhao", "tiktok", "twitter", "weibo", "youtube", "zhihu"]  # 启用的平台
...
source_max_size_mb = 80     # 单个媒体最大下载大小（MB）
source_max_minutes = 8      # 视频最大时长（分钟）
```
